...
function getFizzBuzz(num) {
  const multThree = num % 3 === 0;
  const multFive = num % 5 === 0;
...
function getDigitalRoot(num) {
  const digits = num.toString().split('').map((ch) => +ch);
...
function isBracketsBalanced(str) {
  ...
  // eslint-disable-next-line no-restricted-syntax
  for (const ch of str) {
    if (inBrackets.includes(ch)) {
      stack.push(ch);
    } else if (outBrackets.includes(ch)) {
      const symb = stack.pop();
      if (pairs[symb] !== ch) return false;
    }
  }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathArrays[0].length; i += 1) {
    ...
    for (let j = 1; j < pathArrays.length; j += 1) {
      if (pathArrays[j][i] !== part) {
        let result = pathArrays[0].slice(0, i).join('/');
